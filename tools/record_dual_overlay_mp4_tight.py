#!/usr/bin/env python3
"""record_dual_overlay_mp4_tight.py — visual smoke с TIGHT crop.

Аналогично record_dual_overlay_mp4.py, но crops для DINOv2 inference
извлекаются по tight bbox (X 10-70%, Y 0-55% от original YOLO bbox)
— что симулирует поведение production .so с TightBboxShrinkProbe
+ DetectionProbe restore.

Bbox на overlay рисуется по ORIGINAL (full body) — что соответствует
производственному OSD, видящему restored bbox.
"""
...
PROTOS_NPZ = WORK / "prototypes_dinov2_v1.npz"   # production location (now tight)
DEFAULT_OUT = WORK / "v2_tight_build" / "visual_smoke_v2_tight.mp4"
...
def prep_stretch(crop_bgr):
    """Production-faithful nvinfer-style preprocessing applied to TIGHT crop."""
...
def draw_overlay(frame, dets, frame_idx, fps_actual):
    ...
    line0 = f"TIGHT crop X[{TIGHT_X_LO:.0%}-{TIGHT_X_HI:.0%}] Y[{TIGHT_Y_LO:.0%}-{TIGHT_Y_HI:.0%}]"
    ...
    cv2.putText(out, line0, (8,12), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,255), 1, cv2.LINE_AA)
    cv2.putText(out, line1, (8,26), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1, cv2.LINE_AA)
    cv2.putText(out, line2, (8,40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60,255,60), 1, cv2.LINE_AA)
    cv2.putText(out, line3, (8,52), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60,180,255), 1, cv2.LINE_AA)
    return out
...
def main():
    ...
    print(f"  TIGHT params: X[{TIGHT_X_LO:.0%}-{TIGHT_X_HI:.0%}] Y[{TIGHT_Y_LO:.0%}-{TIGHT_Y_HI:.0%}]")
    print()
    ...
    while frame_idx < total:
        ...
        if results and results[0].boxes is not None:
            ...
            for box, dc in zip(boxes, confs):
                ...
                # Apply TIGHT crop (this is what production .so now does
                # via TightBboxShrinkProbe + nvinfer secondary crop)
                tight = tight_crop_xyxy(frame, [x1,y1,x2,y2])
                if tight.size == 0: continue
                tight_crops_bgr.append(tight)
                valid.append((x1,y1,x2,y2,float(dc)))   # original bbox for overlay
            if tight_crops_bgr:
                pils = [Image.fromarray(cv2.cvtColor(c, cv2.COLOR_BGR2RGB))
                        for c in tight_crops_bgr]
                ...
                for i,(x1,y1,x2,y2,dc) in enumerate(valid):
                    ...
                    prod = parser(trt.infer(prep_stretch(tight_crops_bgr[i])),
                                  protos, classes)
    ...
    print("=" * 78)
    print("=== TIGHT CROP DETECTION STATS ===")
    print("=" * 78)
    ...
    print()
    print(f"  Distribution by color (PROD path = production .so behaviour):")
    print(f"    {'color':<8s} {'n':>6s} {'%':>6s} {'sim_med':>8s} {'sim_min':>8s} {'sim_max':>8s}")
    for c in ["green","yellow","red","blue","unknown"]:
        n = counts_prod[c]; pct = 100*n/max(n_dets_total,1)
        sims = sims_prod[c]
        if sims:
    ...
    for c in ["green","yellow","red","blue","unknown"]:
        n = counts_ref[c]; sims = sims_ref[c]; pct = 100*n/max(n_dets_total,1)
        if sims: print(f"    {c:<8s} {n:>6d} {pct:>5.1f}% {np.median(sims):>8.3f}")
        else:    print(f"    {c:<8s} {n:>6d} {pct:>5.1f}%")
    print()

    stats = {
        "video": args.video, "out_mp4": args.out,
        "tight_params": {"x_lo":TIGHT_X_LO,"x_hi":TIGHT_X_HI,
        ...
        "counts_prod": dict(counts_prod), "counts_ref": dict(counts_ref),
    ...
    print(f"  mp4 (mp4v): {args.out}  ({out_sz_mb:.1f} MB)")
