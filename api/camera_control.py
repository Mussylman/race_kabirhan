...
def _rewrite_resolution(xml_body: str, width: int, height: int) -> tuple[str, tuple[int, int] | None]:
...
def set_camera_resolution(cam: CameraEntry, channel: int,
    ...
    """Return a short status string, e.g. 'OK 1920x1080 → 2560x1440',
    'already 2560x1440', or 'GET FAIL (401)'."""
    ...
    new_xml, old = _rewrite_resolution(r.text, width, height)
    if old == (width, height):
    ...
    return f"OK {old} → {width}x{height}"
...
class ResolutionEnforcer:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="ResolutionEnforcer",
                                        daemon=True)
        self._thread.start()
        log.info("ResolutionEnforcer started — channel=%d %dx%d every %ds on %d cameras",
                 self.channel, self.width, self.height, self.interval, len(self.cameras))
        ...
        while not self._stop.is_set():
            ...
            for cam in self.cameras:
                if self._stop.is_set():
                    break
                status = set_camera_resolution(cam, self.channel, self.width, self.height)
