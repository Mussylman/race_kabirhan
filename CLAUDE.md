...
| `deepstream/src/shm_writer.cpp`, `yolo_parser.cpp`, `color_classifier_parser.cpp` | Hot-path C++ writer. Атомарный seqlock-commit + custom YOLO parser + custom color SGIE parser (DS 9.0 quirk: автопарсер не работает на 2D output — см. `feedback_ds9_sgie_quirks.md`). |
